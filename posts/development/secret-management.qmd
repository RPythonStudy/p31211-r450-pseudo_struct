...
### config.hcl 작성
- 지금까지는 모두 실패했기 때문에 일단 컨테이너 옵션으로 구현하면서 bypass

```
sudo nano projects/vault/config/config.hcl
```
...
listener "tcp" {
  ...
  tls_cert_file = "/vault/certs/vault.crt"       # 볼륨 마운트된 인증서
  tls_key_file  = "/vault/certs/vault.key"       # 볼륨 마운트된 키
